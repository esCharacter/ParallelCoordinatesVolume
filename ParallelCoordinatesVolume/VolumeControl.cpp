...
VolumeControl::VolumeControl(int w, int h)
{
	width = w;
	height = h;
	color[0].r = 1.0; color[0].g = 1.0; color[0].b = 1.0; color[0].a = 0.005;
	color[1].r = 1.0; color[1].g = 0.0; color[1].b = 0.0; color[1].a = 0.015;
	color[2].r = 1.0; color[2].g = 1.0; color[2].b = 0.0; color[2].a = 0.02;
...
//Dim:体数据大小
//********************************************************************//
void VolumeControl::GenerateVolume(int *Data, int *Dim)
{
	GenCube(0, 0, 0, 200, 100, Data, Dim);//大正方体
	GenSphere(100, 100, 100, 80, 200, Data, Dim);//球体
	GenCube(70, 70, 70, 60, 300, Data, Dim);//小正方体
}
void VolumeControl::GenerateVolume1(int *Data, int *Dim)
{
	GenCube(0, 0, 0, 200, 100, Data, Dim);//大正方体
	GenSphere(100, 100, 100, 80, 200, Data, Dim);//球体
	GenCube(70, 70, 70, 60, 300, Data, Dim);//小正方体
}
void VolumeControl::GenerateVolume2(int *Data, int *Dim)
{
	GenCube(0, 0, 0, 200, 100, Data, Dim);//大正方体
	GenCube(50, 50, 50, 100, 200, Data, Dim);//小正方体
	GenCube(10, 10, 10, 80, 300, Data, Dim);//小正方体
}
void VolumeControl::GenerateVolume3(int *Data, int *Dim)
{
	GenCube(0, 0, 0, 200, 100, Data, Dim);//大正方体
	GenCube(80, 80, 80, 80, 200, Data, Dim);//小正方体
	GenSphere(50, 50, 50, 60, 300, Data, Dim);//球体
}
...
//Dim:体数据大小
//********************************************************************//
void VolumeControl::GenCube(int x, int y, int z, int side, int density, int *Data, int *Dim)
{
...
//Dim:体数据大小
//********************************************************************//
void VolumeControl::GenSphere(int x, int y, int z, int radius, int density, int *Data, int *Dim)
{
...
//Dim:体数据大小
//********************************************************************//
void VolumeControl::Classify(float* CData, int *Data, int *Dim)
{
	int *LinePS = Data;
	float *LinePD = CData;
	for (int k = 0; k < Dim[2]; k++)
...
//up:相机朝上的方向
//********************************************************************//
void VolumeControl::RotationMatrix(float *R, float *eye, float *center, float *up)
{
...
//T:平移向量（同上）
//********************************************************************//
void VolumeControl::Composite(float *rgba, int x0, int y0, float *CData, int *Dim, float *R, float *T)
{
	int stepsize = 1;//采样步长
...
//********************************************************************//
bool VolumeControl::Intersection(float *startpos, float *pos, float *dir, int *Dim)
{
...
//Dim:体数据大小
//********************************************************************//
void VolumeControl::TrInterpolation(float *rgba, float *pos, float *CData, int *Dim)
{
...
//Dim:包围盒右上角坐标（左下角坐标为（0,0,0））
//********************************************************************//
bool VolumeControl::CheckinBox(float *point, int *Dim)
{
...
//b:输入向量
//********************************************************************//
void VolumeControl::MatrixmulVec(float *c, float *a, float *b)
{
	float x, y, z;
...
//b:输入向量
//********************************************************************//
void VolumeControl::CrossProd(float *c, float *a, float *b)
{
	float x, y, z;
	x = a[1] * b[2] - b[1] * a[2];
	y = a[2] * b[0] - b[2] * a[0];
...
//a:输入向量
//********************************************************************//
void VolumeControl::Normalize(float *norm, float *a)
{
...
//显示函数
void VolumeControl::display()
{
	glPixelZoom(scaleV, scaleV);
	glDrawPixels(width, height, GL_RGBA, GL_FLOAT, Image);//使用OpenGL的绘图函数
}

int VolumeControl::getWidth()
{
	return width;
}
int VolumeControl::getHeight()
{
	return height;
}
...
void VolumeControl::update()
{
	...
	switch (shape)
	{
	case 1:GenerateVolume1(Data, Dim); break;
	case 2:GenerateVolume2(Data, Dim); break;
	case 3:GenerateVolume3(Data, Dim); break;
	default:
...
void VolumeControl::setColor(ColorV* c)
{
	if (c == NULL)
		return;

	color[0].r = c[0].r; color[0].g = c[0].g; color[0].b = c[0].b; color[0].a = c[0].a;
	color[1].r = c[1].r; color[1].g = c[1].g; color[1].b = c[1].b; color[1].a = c[1].a;
	color[2].r = c[2].r; color[2].g = c[2].g; color[2].b = c[2].b; color[2].a = c[2].a;

}

void VolumeControl::setShape(int i)
{
	shape = i;
}